	...
	if(videourl) {
		...
		if (match&&match[2].length==11){
			...
			$.getJSON('http://gdata.youtube.com/feeds/api/videos/'+video_id+'?v=2&alt=jsonc',function(data,status,xhr){
			    ...
			    var videoduration = parseInt(data.data.duration) - 1;
...
/*
// auth h synarthsh diortwnei to url apo ta google docs
function setQuestionField(url) { 
    var res = url.match(/[-\w]{25,}/);
    if ( res != null ) {
    	$("#question").val("https://docs.google.com/spreadsheet/embeddedform?formkey=" + res)
    }
}
*/
